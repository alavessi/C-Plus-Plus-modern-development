...
{
  std::sort(pointers.begin(), pointers.end(), [] (T* first, T* second) -> bool { return *first < *second; } );
}

template <typename T>
void ReversedCopy(T* source, size_t count, T* destination)
{
  T* rev_copy = new T[count];
  for (size_t i = 0; i < count; i++)
  {
    rev_copy[i] = source[count - 1 - i];
  }
  for (size_t i = 0; i < count; i++)
  {
    destination[i] = rev_copy[i];
  }
  delete [] rev_copy;
}
