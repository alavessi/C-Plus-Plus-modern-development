...
{
    ForwardIterator max = std::find_if(first, last, pred);
    for (ForwardIterator it = max; it != last; it++)
    {
        if (it != max && pred(*it) && *max < *it)
        {
            max = it;
        }
    }
...
void TestSeveralMax()
{
    ...
    const std::forward_list<std::string> text{"One", "two", "Three", "One", "Two", "Three", "one", "Two", "three"};
